...
function validateAnalysisInput(input){
  ...
  if(Array.isArray(steps)){
    for(let i=0;i<steps.length;i++){
      const s=steps[i];
      if(!s?.name) errors.push("funnel step "+(i+1)+" is missing a name");
      if(!isNumber(s?.users)) errors.push("funnel step "+(i+1)+" users must be a non-negative number");
      if(i>0&&isNumber(steps[i-1]?.users)&&isNumber(s?.users)&&s.users>steps[i-1].users) errors.push("funnel step "+(i+1)+" has more users than the previous step");
    }
