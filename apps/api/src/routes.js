...
const router=express.Router();const repo=()=>getRepository();
...
async function updateStore(id,ownerId,patch){if(repo().kind==="mongo")return repo().mongo.updateAnalysis(id,ownerId,patch);const a=memoryGet(id,ownerId);if(!a)return null;Object.assign(a,patch);memoryAnalyses.set(id,a);return a}
async function consumeUsage(userId,limit=2){const period=monthKey();if(repo().kind==="mongo")return repo().mongo.consumeUsage(userId,period,limit);return checkAndConsumeMemory(memoryUsage,userId,limit)}
...
router.post("/analyses/:id/run",async(q,r,n)=>{try{const id=safeId(q.params.id);if(!id)return r.status(400).json({error:"invalid_analysis_id"});const a=await getStore(id,q.userId);if(!a)return r.status(404).json({error:"analysis_not_found"});if(a.status==="completed")return r.status(409).json({error:"analysis_already_completed"});const quota=await consumeUsage(q.userId,2);if(!quota.allowed)return r.status(429).json({error:"monthly_limit_reached",limit:2,used:quota.used});let result;try{result=await buildDiagnostic(a.input)}catch(e){return r.status(502).json({error:"diagnosis_failed"})}const check=validateDiagnosis(result);if(!check.valid)return r.status(422).json({error:"invalid_diagnosis_output",details:check.errors});const updated=await updateStore(a.id,q.userId,{status:"completed",result,completedAt:new Date().toISOString(),usage:quota});r.json(updated||{...a,status:"completed",result,usage:quota})}catch(e){n(e)}});
router.get("/usage",async(q,r,n)=>{try{const period=monthKey();if(repo().kind==="mongo"){const c=await repo().mongo.usage().findOne({userId:q.userId,period}),used=c?.used||0;return r.json({limit:2,used,remaining:Math.max(0,2-used),period})}const used=memoryUsage.get(q.userId+"::"+period)||0;r.json({limit:2,used,remaining:Math.max(0,2-used),period})}catch(e){n(e)}});
router.get("/analyses/:id/report.html",async(q,r,n)=>{try{const id=safeId(q.params.id);if(!id)return r.status(400).json({error:"invalid_analysis_id"});const a=await getStore(id,q.userId);if(!a)return r.status(404).json({error:"analysis_not_found"});if(!a.result)return r.status(409).json({error:"analysis_not_completed"});r.type("html").send(await renderReport(a))}catch(e){n(e)}});
